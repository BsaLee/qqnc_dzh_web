...
async function autoPlantEmptyLands(deadLandIds, emptyLandIds) {
    ...
    // 4. 种植（逐块种植，每块随机延迟2-3秒）
    let plantedLands = [];
    let plantedCount = 0;
    for (let i = 0; i < landsToPlant.length; i++) {
        const landId = landsToPlant[i];
        try {
            const planted = await plantSeeds(actualSeedId, [landId]);
            if (planted > 0) {
                plantedCount++;
                plantedLands.push(landId);
            }
        } catch (e) {
            logWarn('种植', `土地#${landId} 种植失败: ${e.message}`);
        }
        
        // 每块土地种植后随机延迟2-3秒（最后一块不延迟）
        if (i < landsToPlant.length - 1) {
    ...
    if (plantedCount > 0) {
        log('种植', `已在 ${plantedCount} 块地种植 (${plantedLands.join(',')})`, {
            module: 'farm',
            event: 'plant_seed',
            result: 'ok',
            seedId: actualSeedId,
...
function analyzeLands(lands) {
    ...
    for (const land of lands) {
        ...
        const currentPhase = getCurrentPhase(plant.phases, false, landLabel);
        if (!currentPhase) {
            result.empty.push(id);
            continue;
        }
...
async function runFarmOperation(opType) {
    ...
    if (opType === 'all' || opType === 'clear') {
        if (status.needWeed.length > 0 && isAutomationOn('autoWeed')) {
        ...
        if (status.needBug.length > 0 && isAutomationOn('autoBug')) {
        ...
        if (status.needWater.length > 0 && isAutomationOn('autoWater')) {
