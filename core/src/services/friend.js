...
const { getPlantName, getPlantById, getSeedImageBySeedId, getPlantBySeedId } = require('../config/gameConfig');
const { isAutomationOn, getFriendQuietHours, getFriendBlacklist, getStealExcludePlants, addBannedFriend, isBannedFriend } = require('../models/store');
...
function analyzeFriendLands(lands, myGid, friendName = '', excludeSeedIds = []) {
    const result = {
        stealable: [],   // 可偷
    ...
    const excludeSeedSet = new Set(Array.isArray(excludeSeedIds) ? excludeSeedIds : []);
...
async function visitFriend(friend, totalActions, myGid, excludeSeedIds = []) {
    const { gid, name } = friend;

    // 检查是否在封禁黑名单中
    if (isBannedFriend(gid)) {
        log('好友', `跳过封禁好友 ${name}`, {
            module: 'friend', event: 'visit_friend', result: 'skip', friendName: name, friendGid: gid, reason: 'banned'
        });
        return;
    }
    ...
    try {
        ...
        // 检查好友基本信息是否异常（封禁状态）
        const basic = enterReply.basic;
        if (basic) {
            const friendName = basic.name || basic.remark || name || `GID:${gid}`;
            // 检测封禁状态：名字包含特定关键词或状态异常
            const banKeywords = ['封禁', '封号', '违规', '已封', 'banned', 'forbidden', '异常'];
            const isBanned = banKeywords.some(kw => 
                (basic.name && basic.name.toLowerCase().includes(kw)) ||
                (basic.remark && basic.remark.toLowerCase().includes(kw))
            );
            
            if (isBanned) {
                addBannedFriend(gid, friendName, '好友状态显示封禁');
                log('好友', `检测到封禁好友 ${friendName}，已加入黑名单`, {
                    module: 'friend', event: 'banned_friend', result: 'ok', friendName, friendGid: gid
                });
        ...
        const banKeywords = ['封禁', '封号', '违规', 'banned', 'forbidden', '无法访问', '不存在'];
        const isBanned = banKeywords.some(kw => errorMsg.includes(kw));
        
        if (isBanned) {
            addBannedFriend(gid, name, '无法访问好友农场');
            log('好友', `访问好友 ${name} 失败，已加入黑名单`, {
                module: 'friend', event: 'banned_friend', result: 'ok', friendName: name, friendGid: gid, error: e.message
            });
        } else {
    ...
    const status = analyzeFriendLands(lands, myGid, name, excludeSeedIds);
...
async function checkFriends() {
    ...
    try {
        ...
        const excludeSeedIds = getStealExcludePlants();
        ...
        for (const f of friends) {
            ...
            // 跳过蒙面偷菜的好友（名字包含"蒙面"或GID为0或无效）
            if (name.includes('蒙面') || gid <= 0) continue;
        ...
        for (let i = 0; i < friendsToVisit.length; i++) {
            ...
            try {
                await visitFriend(friend, totalActions, state.gid, excludeSeedIds);
            } catch {
                // 单个好友访问失败不影响整体
            }
