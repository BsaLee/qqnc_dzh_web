...
function checkAndResetDaily() {
    ...
    if (todayStats.date !== today) {
        ...
        todayStats.harvestGold = 0;
        todayStats.stealSellGold = 0;
    }
}

function recordHarvestGold(amount) {
    checkAndResetDaily();
    todayStats.harvestGold += amount;
}

function recordStealSellGold(amount) {
    checkAndResetDaily();
    todayStats.stealSellGold += amount;
}
...
function getStats(statusData, userState, connected, limits) {
    ...
    return {
        ...
        todayStats: {
            harvestGold: todayStats.harvestGold,
            stealSellGold: todayStats.stealSellGold,
            total: todayStats.harvestGold + todayStats.stealSellGold,
        },
        limits,
    };
}
