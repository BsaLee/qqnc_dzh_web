...
const { toLong, toNum, log, logWarn, sleep, getDateKey } = require('../utils/utils');
const { updateStatusGold } = require('./status');
const { recordHarvestGold, recordStealSellGold } = require('./stats');
 ...
 * 检查并出售所有果实
 */
async function sellAllFruits(source = 'harvest') {
    ...
    try {
        ...
        if (totalGoldEarned > 0) {
            ...
            // 记录今日金币收益
            if (source === 'steal') {
                recordStealSellGold(totalGoldEarned);
            } else {
                recordHarvestGold(totalGoldEarned);
            }
        }
    } catch (e) {
