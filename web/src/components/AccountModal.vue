...
<template>
  <div v-if="show" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
    <div class="max-w-md w-full overflow-hidden rounded-lg bg-white/80 backdrop-blur-md shadow-xl dark:bg-gray-800/80">
      ...
      <div class="p-4">
        ...
        <div v-if="activeTab === 'qr'" class="flex flex-col items-center justify-center py-4 space-y-4">
          ...
          <div v-if="qrData && (qrData.image || qrData.qrcode)" class="border rounded bg-white/40 backdrop-blur-md p-2">
  ...
  <!-- Success Tip Modal -->
  <div v-if="showSuccessTip" class="fixed inset-0 z-[60] flex items-center justify-center bg-black/50">
    <div class="max-w-sm w-full mx-4 rounded-lg bg-white p-6 shadow-xl dark:bg-gray-800">
      <div class="text-center">
        <div class="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-full bg-green-100 dark:bg-green-900/30">
          <div class="i-carbon-checkmark text-2xl text-green-600 dark:text-green-400" />
        </div>
        <h3 class="text-lg font-medium text-gray-900 dark:text-white">
          账号添加成功
        </h3>
        <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">
          请按需求在设置页面配置好功能后，点击开启托管按钮开始运行
        </p>
      </div>
      <div class="mt-6">
        <BaseButton
          variant="primary"
          block
          @click="showSuccessTip = false; close()"
        >
          我知道了
        </BaseButton>
      </div>
