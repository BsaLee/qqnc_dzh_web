...
<template>
  <div class="space-y-4">
    ...
    <div v-else-if="statusError" class="border border-red-200 rounded-lg bg-red-50/40 backdrop-blur-md p-8 text-center text-red-500 shadow dark:border-red-800 dark:bg-red-900/20">
    ...
    <div v-else-if="items.length === 0" class="rounded-lg bg-white/40 backdrop-blur-md p-8 text-center text-gray-500 shadow dark:bg-gray-800/40">
      无可展示物品
    </div>
    ...
    <div v-else class="grid grid-cols-2 gap-4 lg:grid-cols-5 md:grid-cols-4 sm:grid-cols-3 xl:grid-cols-6">
      <div
        v-for="item in items"
        :key="item.id"
        class="group relative flex flex-col items-center border rounded-lg bg-white/40 backdrop-blur-md p-3 transition dark:border-gray-700 dark:bg-gray-800/40 hover:shadow-md"
      >
