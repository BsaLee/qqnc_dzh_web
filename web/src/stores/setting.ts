...
export const useSettingStore = defineStore('setting', () => {
  ...
  async function fetchSettings(accountId: string) {
    ...
    try {
      ...
      if (data && data.ok && data.data) {
        ...
        settings.value.plantDelaySeconds = d.plantDelaySeconds || 1
        ...
        settings.value.stealExcludePlants = Array.isArray(d.stealExcludePlants) ? d.stealExcludePlants : []
      }
    }
    finally {
      loading.value = false
  ...
  async function saveStealExcludePlants(accountId: string, plants: number[]) {
    if (!accountId)
      return { ok: false, error: '未选择账号' }
    loading.value = true
    try {
      const { data } = await api.post('/api/steal-exclude-plants', { plants })
      if (data && data.ok) {
        settings.value.stealExcludePlants = plants
        return { ok: true }
      }
      return { ok: false, error: '保存失败' }
    }
  ...
  return { settings, loading, fetchSettings, saveSettings, saveOfflineConfig, changeAdminPassword, saveStealExcludePlants }
})
