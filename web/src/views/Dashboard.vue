...
import { getOpName, getOpIcon, getOpColor, getLogTagClass, getLogMsgClass } from '@/constants/operations'
...
// 防抖处理刷新操作
const debouncedRefresh = useDebounceFn(async (forceReloadLogs = false) => {
  if (currentAccountId.value) {
...
async function refresh(forceReloadLogs = false) {
  await debouncedRefresh(forceReloadLogs)
}
...
async function clearLogs() {
  ...
  try {
    await fetch(`/api/logs?accountId=${currentAccountId.value}`, {
      method: 'DELETE',
    })
...
const { pause: pauseRefresh, resume: resumeRefresh } = useIntervalFn(() => {
  // 只在实时连接断开或有筛选条件时才进行定时刷新
  if (!realtimeConnected.value || hasActiveLogFilter.value) {
    refresh()
  }
}, 15000) // 延长刷新间隔到 15 秒

// Countdown timer (every 1s)
const { pause: pauseCountdown, resume: resumeCountdown } = useIntervalFn(updateCountdowns, 1000)
...
<template>
  <div class="flex flex-col gap-6 pt-6 md:h-full">
    ...
    <div class="grid grid-cols-1 gap-4 lg:grid-cols-3 sm:grid-cols-2">
      <!-- Account & Exp -->
      <div class="flex flex-col rounded-lg bg-white/40 backdrop-blur-md p-4 shadow dark:bg-gray-800/40">
      ...
      <!-- Assets & Status -->
      <div class="flex flex-col justify-between rounded-lg bg-white/40 backdrop-blur-md p-4 shadow dark:bg-gray-800/40">
        ...
        <div class="mt-3 border-t border-gray-100 pt-3 dark:border-gray-700">
          <div class="flex items-center justify-between text-xs">
            <span class="text-gray-500">种菜收获：</span>
            <span class="text-yellow-600 font-medium">{{ status?.todayStats?.harvestGold || 0 }}</span>
          </div>
          <div class="mt-1 flex items-center justify-between text-xs">
            <span class="text-gray-500">偷菜售卖：</span>
            <span class="text-yellow-600 font-medium">{{ status?.todayStats?.stealSellGold || 0 }}</span>
          </div>
          <div class="mt-2 flex items-center justify-between border-t border-gray-100 pt-2 text-xs dark:border-gray-700">
            <span class="text-gray-500 font-medium">今日合计：</span>
            <span class="text-green-600 font-bold">{{ status?.todayStats?.total || 0 }}</span>
          </div>
        </div>
        <div class="mt-3 border-t border-gray-100 pt-3 dark:border-gray-700">
    ...
    <div class="flex flex-1 flex-col items-stretch gap-6 md:flex-row md:overflow-hidden">
      ...
      <div class="flex flex-1 flex-col gap-6 md:w-3/4 md:overflow-hidden">
        <!-- Logs -->
        <div class="flex flex-1 flex-col rounded-lg bg-white/40 backdrop-blur-md p-6 shadow md:overflow-hidden dark:bg-gray-800/40">
          ...
          <div ref="logContainer" class="max-h-[50vh] min-h-0 flex-1 overflow-y-auto rounded bg-gray-50/40 backdrop-blur-md p-4 text-sm leading-relaxed font-mono md:max-h-none dark:bg-gray-900/40" @scroll="onLogScroll">
      ...
      <div class="flex flex-col gap-6 md:w-1/4">
        <!-- Next Checks -->
        <div class="flex flex-col rounded-lg bg-white/40 backdrop-blur-md p-6 shadow dark:bg-gray-800/40">
        ...
        <!-- Operations Grid -->
        <div class="flex-1 rounded-lg bg-white/40 backdrop-blur-md p-4 shadow dark:bg-gray-800/40">
          ...
          <div v-else class="grid grid-cols-2 gap-2 2xl:gap-3">
            <div
              ...
              :key="key"
              class="flex items-center justify-between rounded bg-gray-50/40 backdrop-blur-md px-3 py-2 dark:bg-gray-700/30"
            >
              <div class="flex items-center gap-2">
